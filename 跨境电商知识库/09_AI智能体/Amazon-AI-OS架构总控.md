...
- 机器事实源：`.codex/amazon-ai-os.config.json`。
- 长期知识事实源：`跨境电商知识库` Obsidian Vault。
- 根目录 `00_体系总览.md` 至 `06_测试案例与验收清单.md` 是训练、说明和验收资料，不作为机器调度的唯一事实源。
- 飞书知识库是上游协作来源，必须先同步或摘录到 Obsidian Markdown，再进入长期知识层。
- 岗位覆盖以 [[岗位注册覆盖对照表]] 为准；正式调度只调用已注册 Agent。
- 端到端验收以 [[端到端验收记录]] 为准，真实业务动作仍需人工审批。
